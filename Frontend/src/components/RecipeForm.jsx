...
function RecipeForm({ initialValues, onSubmit, title = "Share Your Recipe", buttonText = "Publish Recipe" }) {
    ...
    const formik = useFormik({
        ...
        validate: (values) => {
            const { error } = postRecipeValidationSchema.validate(values, { abortEarly: false });
            if (!error) return {};
    ...
    return (
        <form onSubmit={formik.handleSubmit} className="bg-white border border-slate-200 p-8 rounded-2xl w-full max-w-[550px] flex flex-col gap-5 shadow-2xl mb-10">
            ...
            <div className="flex flex-col gap-1">
                ...
                />
                {formik.touched.title && formik.errors.title && <span className="text-red-500 text-[10px]">{formik.errors.title}</span>}
            </div>

            {/* Description */}
            <div className="flex flex-col gap-1">
                <textarea
                    className={`border p-2 rounded outline-amber-400 text-sm h-20 resize-none ${formik.touched.description && formik.errors.description ? 'border-red-500' : ''}`}
                    name="description"
                    ...
                    onBlur={formik.handleBlur}
                />
                {formik.touched.description && formik.errors.description && <span className="text-red-500 text-[10px]">{formik.errors.description}</span>}
            </div>
            ...
            <div className="flex flex-col gap-1">
                <input
                    ...
                    onBlur={formik.handleBlur}
                />
                {formik.touched.img && formik.errors.img && <span className="text-red-500 text-[10px]">{formik.errors.img}</span>}
            </div>
                ...
                {formik.values.ingredients.map((ing, i) => (
                    <div key={i} className="flex gap-2 items-center">
                        ...
                        {formik.values.ingredients.length > 1 && (
                            <button type="button" onClick={() => removeArrayItem("ingredients", i)} className="text-red-500 text-sm">✕</button>
                        )}
                    </div>
                ))}
            </div>
                ...
                {formik.values.instructions.map((ins, i) => (
                    <div key={i} className="flex gap-2 items-center">
                        ...
                        {formik.values.instructions.length > 1 && (
                            <button type="button" onClick={() => removeArrayItem("instructions", i)} className="text-red-500 text-sm">✕</button>
                        )}
                    </div>
                ))}
            </div>
            ...
            <div className="flex gap-2 font-semibold">
                <select className="border p-2 rounded outline-amber-400 text-sm flex-1 bg-white cursor-pointer" name="level" value={formik.values.level} onChange={formik.handleChange}>
                    {levels.map((l) => (
                        <option key={l} value={l}>{l.charAt(0).toUpperCase() + l.slice(1)}</option>
                    ))}
                </select>
                <select className="border p-2 rounded outline-amber-400 text-sm flex-1 bg-white cursor-pointer" name="category" value={formik.values.category} onChange={formik.handleChange}>
                    {categories.map((c) => (
