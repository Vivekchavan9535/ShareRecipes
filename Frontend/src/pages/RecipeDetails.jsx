...
function RecipeDetails() {
	...
	return (
		<main className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
					...
					<div className="flex items-center gap-4">
						...
						<div className="flex items-center gap-2">
							{user && !isOwner && (
								...
								>
									{isFavorite ? <span className="bg-rose-50 text-rose-600 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest flex items-center gap-1  border-black border-2">Remove from Favorites</span> : <span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest flex items-center gap-1 border-black border-2">Add to Favorites</span>}
								</button>
							)}
							{recipe.avgRating > 0 && (
			...
			<button className="bg-white border-2 rounded px-2 mt-5" onClick={() => { window.print() }}>Print Recipe 🖨️</button>
		</main >
	);
}
