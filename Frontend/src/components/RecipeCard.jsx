		...
		<main onClick={() => onclick()} className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer group">
			...
			<section className="h-48 w-full bg-gray-200 overflow-hidden relative">
				...
				<div className="absolute top-3 right-3 flex flex-col gap-2 items-end">
					<span className="bg-emerald-500 text-white text-[10px] uppercase tracking-widest font-black px-2.5 py-1 rounded-lg shadow-lg">
						{recipes.category}
					</span>
			...
			<section className="p-4">
				<div className="flex justify-between items-start mb-2">
					<h2 className="text-xl font-bold text-slate-900 line-clamp-1 group-hover:text-emerald-600 transition-colors">
						{recipes.title}
					</h2>
				</div>
				...
				<div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-100">
					<div className="flex items-center gap-2 text-slate-500 text-xs font-semibold">
						<span className="bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-md uppercase tracking-tighter">
							{recipes.level}
						</span>
						<span>•</span>
					...
					<div className="flex items-center gap-1 text-sm font-semibold">
						{Array.from({ length: 5 }, (_, index) => (
							<span key={index} className={index < Math.round(recipes.avgRating || 0) ? "text-amber-400" : "text-slate-200"}>★</span>
						))}
